...
class Tree(object):
    ...
    def entropy(Y):
        ...
        e = 0
        for a_cat in cats:
            e += (cats[a_cat] / num_elements) * math.log2((cats[a_cat] / num_elements)) * -1
        ...
        for a_cat in cats:
            ...
            for i in range(0, len(X)):
                if a_cat == Y[i]:
                    temp_count += 1
                    sub_list.append(X[i])
            inner_cats[a_cat] = (Counter(sub_list), temp_count)

        for a_cat in cats:
            inner_prob = 0
            part = inner_cats.get(a_cat)
            for an_x in inner_cats[a_cat][0]:
                inner_prob += (part[0][an_x] / part[1]) * math.log2((part[0][an_x] / part[1])) * -1

            ce += (cats[a_cat] / num_elements) * inner_prob
